...
import { analyzeHandler, discoverHandles, type AnalysisResult, type DiscoveredHandle } from "@fearless/aot-analyzer";
...
export function compileApp(options: CompileAppOptions): CompileAppResult {
  ...
  // Discover module-scope `const db = fearless.sql("primary")` declarations
  // ONCE per file so each handler's transpile sees the same handle set.
  const discoveredHandlesForFile = discoverHandles(sourceFile, ts);
      ...
      if (ts.isPropertyAccessExpression(callee)) {
        ...
        if (httpMethod != null && node.arguments.length >= 2) {
          ...
          if (
            ...
            const route = discoverHandlerRoute(httpMethod, path, handlerArg, aotIdCounter, discoveredHandlesForFile);
            routes.push(route);
  ...
  const aotRoutes = routes.filter((r): r is DiscoveredRoute & { kind: "aot" } => r.kind === "aot");
  if (aotRoutes.length > 0) {
    ...
    // Sync handlers compile unconditionally. Async handlers reference
    // `crate::aot::handles::HandleRegistry` and the deadpool pool — both gated
    // behind the `pg-handles` feature. We mirror the gate on the generated fn
    // and on every wrapper that adapts it to `AotAsyncHandlerFn`.
    for (const route of aotRoutes) {
      if (route.transpile.kind === "async") {
        rustChunks.push("#[cfg(feature = \"pg-handles\")]");
        rustChunks.push(route.transpile.rustSource);
        rustChunks.push("");
        // Per-route adapter that converts the AotAsyncHandlerFn signature into
        // a borrow of the AotRequest the user handler expects, and bumps the
        // borrowed `&HandleRegistry` out of the `Arc<>` for the call. The
        // `async move` captures the owned param maps so they outlive the
        // borrowed AotRequest across the await.
        const fnName = route.transpile.fnName;
        rustChunks.push("#[cfg(feature = \"pg-handles\")]");
        rustChunks.push(`fn ${fnName}_dispatch(`);
        rustChunks.push("    method: String,");
        rustChunks.push("    path: String,");
        rustChunks.push("    params: std::collections::HashMap<String, String>,");
        rustChunks.push("    query: std::collections::HashMap<String, String>,");
        rustChunks.push("    headers: std::collections::HashMap<String, String>,");
        rustChunks.push("    handles: std::sync::Arc<crate::aot::handles::HandleRegistry>,");
        rustChunks.push(") -> std::pin::Pin<Box<dyn std::future::Future<Output = Vec<u8>> + Send>> {");
        rustChunks.push("    Box::pin(async move {");
        rustChunks.push("        let req = crate::aot::runtime::AotRequest {");
        rustChunks.push("            method: method.as_str(),");
        rustChunks.push("            path: path.as_str(),");
        rustChunks.push("            url: path.as_str(),");
        rustChunks.push("            ip: \"\",");
        rustChunks.push("            params: &params,");
        rustChunks.push("            query: &query,");
        rustChunks.push("            headers: &headers,");
        rustChunks.push("        };");
        rustChunks.push(`        ${fnName}(&req, &*handles).await`);
        rustChunks.push("    })");
        rustChunks.push("}");
        rustChunks.push("");
      } else {
    ...
    for (const route of aotRoutes) {
      ...
      if (route.transpile.kind === "async") {
        // `add_async` only exists when `pg-handles` is on; gate the registration
        // accordingly. With pg-handles off, the route silently disappears from
        // the table and the runtime serves a 404 (matches the analyzer's
        // intent: this build needs the pg feature to function).
        rustChunks.push("    #[cfg(feature = \"pg-handles\")]");
        rustChunks.push(`    table.add_async(${method}, ${path}, ${route.transpile.fnName}_dispatch);`);
      } else {
  ...
  const aotAsyncRoutes = aotRoutes.filter((r) => r.transpile.kind === "async");
...
function discoverHandlerRoute(
  ...
  idCounter: number,
  discoveredHandlesForFile: readonly DiscoveredHandle[]
): DiscoveredRoute {
  ...
  const outcome = transpileHandler({
    ...
    path,
    id,
    discoveredHandles: discoveredHandlesForFile,
  });
  if (!outcome.success) {
    return {
      kind: "bun",
